...
4.（2分）以比较作为基本运算，在 $N$ 个数中找出最大数，最坏情况下所需要的最少的比较次数为（　）。

   A. $N^2$　　B. $N$　　C. $N-1$　　D. $N+1$

5.（2分）对于入栈顺序为 `a, b, c, d, e` 的序列，下列（　）不是合法的出栈序列。

   A. `a, b, c, d, e`　　B. `e, d, c, b, a`　　C. `b, a, c, d, e`　　D. `c, d, a, e, b`

6.（2分）对于有 $n$ 个顶点、$m$ 条边的无向连通图 $(m>n)$，需要删掉（　）条边才能使其成为一棵树。

   A. $n-1$　　B. $m-n$　　C. $m-n-1$　　D. $m-n+1$

7.（2分）二进制数 $101.11$ 对应的十进制数是（　）。

   A. 6.5　　B. 5.5　　C. 5.75　　D. 5.25

8.（2分）如果一棵二叉树只有根结点，那么这棵二叉树高度为 1。请问高度为 5 的完全二叉树有（　）种不同的形态？

   A. 16　　B. 15　　C. 17　　D. 32

9.（2分）表达式 `a*(b+c)*d` 的后缀表达式为（　），其中 `*` 和 `+` 是运算符。

   A. `**a+bcd`　　B. `abc+*d*`　　C. `abc+d**`　　D. `*a*+bcd`

10.（2分）6 个人，两个人组一队，总共组成三队，不区分队伍的编号。不同的组队情况有（　）种。
...
12.（2分）由 1，1，2，2，3 这五个数字组成不同的三位数有（　）种。
...
14.（2分）以 `a` 为起点，对右边的无向图进行深度优先遍历，则 `b`、`c`、`d`、`e` 四个点中有可能作为最后一个遍历到的点的个数为（　）。（图中边为：`a-b`、`a-c`、`b-d`、`c-d`、`c-e`。）

   A. 1　　B. 2　　C. 3　　D. 4

15.（2分）有四个人要从 A 点坐一条船过河到 B 点，船一开始在 A 点。该船一次最多可坐两个人。已知这四个人中每个人独自坐船的过河时间分别为 1，2，4，8，且两个人坐船的过河时间为两人独自过河时间的较大者。则最短（　）时间可以让四个人都过河到 B 点（包括从 B 点把船开回 A 点的时间）。
...
16.（1.5分）输入的 `n` 等于 1001 时，程序不会发生下标越界。（　）

17.（1.5分）输入的 `a[i]` 必须全为正整数，否则程序将陷入死循环。（　）

18.（1.5分）当输入为 `5 2 11 9 16 10` 时，输出为 `3 4 3 17 5`。（　）

19.（1.5分）当输入为 `1 511998` 时，输出为 `18`。（　）

20.（1.5分）将源代码中 `g` 函数的定义（14-17 行）移到 `main` 函数的后面，程序可以正常编译运行。（　）
...
25.（3分）设输入字符串长度为 `n`，`decode` 函数的时间复杂度为（　）。
...
28.（1.5分）若输入不为 `1`，把第 13 行删去不会影响输出的结果。（　）
...
31.（3分）`init` 函数的时间复杂度为（　）。

   A. $\Theta(n)$　　B. $\Theta(n\log n)$　　C. $\Theta(n\sqrt n)$　　D. $\Theta(n^2)$

32.（3分）在执行完 `init()` 后，`f[1]`，`f[2]`，`f[3]` …… `f[100]` 中有（　）个等于 2。
...
66              if (____⑤____ && binary_search(A, n, A[i].x, A[j].y) &&
...
42.（3分）④处应填（　）
   ...
   A. `!cmp(A[mid], p)`　　B. `cmp(A[mid], p)`　　C. `cmp(p, A[mid])`　　D. `!cmp(p, A[mid])`
