...
- 数据分析支持初赛/复赛场次筛选；复赛从 `CSPJ25T1` / `CSPS25T4` 这类公开编程题中统计 T1-T4 题位、难度和考点热力数据，不计算考点权重。
...
- 统一入口为 `./start.sh`。
- `start.sh` 支持安装、启动、停止、重启、日志、状态、数据卷查看、备份和恢复。
...
- 部署文档补充了脚本式全量备份/恢复、`.env` 恢复、清空 Docker 数据、只清空用户数据、只清空编程题库和只清空初赛题库的命令。

## 本次清理

- 删除 README 中不存在的入口脚本说明。
- 删除题面编辑里新增测试点的遗留前端函数。
- 删除前端未使用的 Markdown 预览辅助函数和初赛旧表格行渲染函数。
- 移除历史 `scoring_mode` 的新库 schema、迁移和题号重命名绕路逻辑；旧库中已有列会被自然忽略。
- 删除文档中输出比较、浮点误差等已不作为 UI 功能暴露的说明。
- 重写 README、架构文档、部署文档、开发文档、使用手册和收尾检查。
- 补全 API 清单和环境变量说明。
- 修正题面自定义 Markdown 指令、拆行图片链接预览和附件重命名逻辑。
- 增加 `start.sh backup`、`start.sh restore` 和 `start.sh data-volume`，避免手写 Docker volume 名导致恢复无效。
- 版本号更新到 `1.5.0`，包含 DeepSeek V4 显式适配、讯飞单向备用、角色感知教学审查、生成中断记录和按用户用量统计。
