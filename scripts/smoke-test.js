...
assert.strictEqual(packageJson.version, '1.5.0', 'package version should be 1.5.0 after DeepSeek V4 and 小轻 usage analytics');
...
assert(dbJsFinal.includes('CREATE TABLE IF NOT EXISTS app_settings') && dbJsFinal.includes('CREATE TABLE IF NOT EXISTS ai_sessions') && dbJsFinal.includes('CREATE TABLE IF NOT EXISTS ai_messages') && dbJsFinal.includes('CREATE TABLE IF NOT EXISTS ai_usage_events'), 'database migration should create AI settings, sessions, messages, and usage events');
...
assert(appJs.includes('name="reviewEnabled"') && appJs.includes('name="reviewProvider"') && appJs.includes('name="reviewPrompt"') && appJs.includes('二次审查提示词') && appJs.includes('name="maxHistoryMbPerUser"'), 'AI admin page should expose independent review provider settings and history quota');
assert(appJs.includes('renderAdminAiUsage') && appJs.includes('/api/admin/ai-usage') && appJs.includes("path === '/admin/ai/usage'"), 'AI admin should provide per-user usage analytics');
assert(appJs.includes('requestsUsedToday') && appJs.includes('todayUsage') && appJs.includes('ai-message-badge interrupted'), 'AI chat should show daily usage and interrupted message states');
...
const aiProviderJs = fs.readFileSync(path.join(__dirname, '..', 'backend', 'ai-provider.js'), 'utf8');
assert(settingsJs.includes("'ai.provider': 'xfyun'") && settingsJs.includes("'ai.deepseek_model': AI_PROVIDER_DEFAULTS.deepseek.model") && settingsJs.includes("'ai.context_mode': 'recent'") && settingsJs.includes("'ai.context_recent_messages': '6'"), 'AI settings should keep Xunfei as default and provide DeepSeek V4 Flash with recent context');
assert(settingsJs.includes("'ai.review_enabled': '1'") && settingsJs.includes("'ai.review_provider': 'xfyun'") && settingsJs.includes("'ai.review_prompt': AI_REVIEW_PROMPT") && settingsJs.includes('reviewPrompt'), 'AI settings should persist independent second-pass review settings');
...
assert(settingsJs.includes('AI_PROVIDER_DEFAULTS') && settingsJs.includes('XFYUN_API_KEY') && settingsJs.includes('DEEPSEEK_API_KEY'), 'AI settings should support Xunfei and DeepSeek without storing keys');
assert(aiProviderJs.includes('/chat/completions') && aiProviderJs.includes('stream: true') && aiProviderJs.includes('stream_options') && aiProviderJs.includes('include_usage: true'), 'AI provider adapter should request OpenAI-compatible streams and DeepSeek usage data');
assert(aiProviderJs.includes("body.thinking = { type:") && aiProviderJs.includes('reasoning_effort') && aiProviderJs.includes('body.user_id = privateUserId'), 'DeepSeek adapter should explicitly configure thinking and pseudonymous user isolation');
assert(aiRoutesJs.includes('runGeneration') && aiRoutesJs.includes('reviewMessages') && aiRoutesJs.includes('settings.reviewEnabled'), 'AI route should perform optional hidden-draft second-pass review');
...
assert(aiRoutesJs.includes('【当前原始问题】') && aiRoutesJs.includes('【最近对话，仅供理解】') && aiRoutesJs.includes('【待审查草稿】'), 'AI review should receive the original request, recent context, user role, and hidden draft');
assert(aiProviderJs.includes('body.enable_thinking = false'), 'Xunfei streaming should disable reasoning output so the UI receives assistant content directly');
assert(aiRoutesJs.includes("settings.provider === 'deepseek'") && aiRoutesJs.includes("provider: 'xfyun'") && aiRoutesJs.includes('err.canFallback'), 'DeepSeek generation should fall back one-way to Xunfei only for supported failures');
assert(aiRoutesJs.includes('recordUsageEvent') && aiRoutesJs.includes("router.get('/usage'") && adminRoutesJs.includes("router.get('/ai-usage'"), 'AI calls should persist user and admin usage analytics');
...
assert(aiRoutesJs.includes("sse(res, 'stage'") && aiRoutesJs.includes('小轻思考中') && aiRoutesJs.includes('小轻正在整理回答') && !aiRoutesJs.includes(retiredAnalysisStageLabel) && !aiRoutesJs.includes(retiredReviewStageLabel), 'AI route should distinguish generation and hidden-draft review without exposing policy internals');
...
assert(aiPromptsJs.includes('AI_REVIEW_PROMPT') && aiPromptsJs.includes('最终回复审查器') && aiPromptsJs.includes('只输出最终给用户看的回复'), 'AI prompt policy should include a strict configurable final-response reviewer');
assert(AI_REVIEW_PROMPT.includes('普通学生明显索要完整答案') && AI_REVIEW_PROMPT.includes('当前用户为管理员') && AI_REVIEW_PROMPT.includes('Markdown 结构必须完整'), 'default review prompt should apply role-aware teaching boundaries and preserve Markdown');
...
assert(styleCss.includes('.ai-session-tools {\n  display: none;') && styleCss.includes('grid-template-columns: minmax(0, 1fr) auto auto auto;') && styleCss.includes('.ai-sidebar.ai-multiselect-mode .ai-session-tools { display: grid;') && styleCss.includes('.ai-sidebar.ai-multiselect-mode .ai-session-check-wrap { display: grid;'), 'AI multi-select controls should stay hidden until enabled and fit within the sidebar');
