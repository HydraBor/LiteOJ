...
│   ├── languages.js           # 语言配置
│   ├── sandbox.js             # legacy local executor 的 host/docker 包装
│   └── checker.js             # 输出比较
├── seed/
│   ├── problems/              # 初始化编程题
...
默认生产部署使用 go-judge：Web 跑在 Docker Compose `app` 容器中，`go-judge` 作为只绑定 `127.0.0.1` 的特权容器提供受限执行环境，judge worker 跑在宿主机上并只通过 HTTP 调用 go-judge。代码级 `local` 执行器和 `host/docker` 沙箱仅保留给本地开发或可信内网 fallback。
...
- 使用 HTTPS。
- 修改 `.env` 中的 `JWT_SECRET`、`JUDGE_TOKEN` 和 `ADMIN_PASSWORD`；使用 `./start.sh` 时脚本会自动生成。
- Web 服务放在 Nginx 反向代理后面。
- 公网同机部署时使用宿主机 judge + go-judge，不要使用 `JUDGE_EXECUTOR=local` 或容器内 `host` judge 处理陌生用户提交。
...
本版完善了编程题评测能力：新增 OI/ACM 评分方式、子任务分组、常用输出比较模式，并把生产评测执行器切换为 go-judge；同时延续数据分析模块、账号安全和个人主页改密能力。
