...
1.（2分）32 位 `int` 类型的存储范围是？（　）

   A. -2147483647 ~ +2147483647　　B. -2147483647 ~ +2147483648　　C. -2147483648 ~ +2147483647　　D. -2147483648 ~ +2147483648

2.（2分）计算 $(14_8-1010_2)\times D_{16}-1101_2$ 的结果，并选择答案的十进制值：（　）

   A. 13　　B. 14　　C. 15　　D. 16

3.（2分）某公司有 10 名员工，分为 3 个部门：A 部门有 4 名员工、B 部门有 3 名员工、C 部门有 3 名员工。现需要从这 10 名员工中选出 4 名组成一个工作小组，且每个部门至少要有 1 人。问有多少种选择方式？（　）

   A. 120　　B. 126　　C. 132　　D. 238

4.（2分）以下哪个序列对应数字 0 至 8 的 4 位二进制格雷码（Gray code）？（　）
...
5.（2分）记 1KB 为 1024 字节（byte）、1MB 为 1024KB，那么 1MB 是多少二进制位（bit）？（　）

   A. 1000000　　B. 1048576　　C. 8000000　　D. 8388608

6.（2分）以下哪个不是 C++ 中的基本数据类型？（　）

   A. `int`　　B. `float`　　C. `struct`　　D. `char`

7.（2分）以下哪个不是 C++ 中的循环语句？（　）

   A. `for`　　B. `while`　　C. `do-while`　　D. `repeat-until`

8.（2分）在 C/C++ 中，`(char)('a'+13)` 与下面的哪一个值相等？（　）

   A. `'m'`　　B. `'n'`　　C. `'z'`　　D. `'3'`

9.（2分）假设有序表中有 1000 个元素，则用二分法查找元素 X 最多需要比较（　）次。

   A. 25　　B. 10　　C. 7　　D. 1

10.（2分）下面的哪一个不是操作系统名字？（　）
...
12.（2分）已知二叉树的前序遍历为 `[A, B, D, E, C, F, G]`，中序遍历为 `[D, B, E, A, F, C, G]`，请问该二叉树的后序遍历结果是？（　）
...
13.（2分）给定一个空栈，支持入栈和出栈操作。若入栈操作的元素依次是 1 2 3 4 5 6，其中 1 最先入栈、6 最后入栈，下面哪种出栈顺序是不可能的？（　）

   A. 6 5 4 3 2 1　　B. 1 6 5 4 3 2　　C. 2 4 6 5 3 1　　D. 1 3 5 2 4 6

14.（2分）有 5 个男生和 3 个女生站成一排，规定 3 个女生必须相邻。问有多少种不同的排列方式？（　）

   A. 4320 种　　B. 5040 种　　C. 3600 种　　D. 2880 种
...
17.（1.5分）若将 `isPrime` 函数中的循环条件改为 `i <= n / 2`，输入 `"20"` 时，`countPrimes(20)` 的输出将变为 `"6"`。（　）

18.（1.5分）`sumPrimes` 函数计算的是从 2 到 n 之间的所有素数之和。（　）
...
21.（1.5分）当输入的 `cost` 数组为 `{10, 15, 20}` 时，程序的输出为 15。（　）

22.（1.5分）如果将 `dp[i-1]` 改为 `dp[i-3]`，程序可能会产生编译错误。（　）

23.（2分）程序总是输出 `cost` 数组中最小的元素。（　）

- 单选题

24.（3分）当输入的 `cost` 数组为 `{1, 100, 1, 1, 1, 100, 1, 1, 100, 1}` 时，程序的输出为（　）。
...
26.（3分）若将代码中的 `min(dp[i-1], dp[i-2]) + cost[i-1]` 修改为 `dp[i-1] + cost[i-2]`，输入 `cost` 数组为 `{5, 10, 15}` 时，程序的输出为（　）。
...
28.（1.5分）当 `b` 为负数时，`customFunction(a, b)` 会陷入无限递归。（　）

29.（1.5分）当 `b` 的值越大，程序的运行时间越长。（　）
...
31.（3分）如果输入 `x=3` 和 `y=3`，则程序的最终输出为（　）。
...
12          return;
13      }
14      dfs(i - 1, ____③____);
15      move(src, tgt);
16      dfs(____⑤____, ____④____);
17  }
18
19  int main() {
20      int n;
